...
export function EventManagePage() {
  ...
  const fetch = async () => {
    setLoading(true);
    try {
      const r = await api<{ events: Event[]; total_count: number }>(
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-gray-50">
                ...
                <th className="text-left px-4 py-2">作成者</th>
                <th className="text-left px-4 py-2">期限</th>
            ...
            <tbody>
              {events.map((e) => (
                <tr key={e.id} className="border-b last:border-0">
                  ...
                  <td className="px-4 py-2 text-gray-500">{e.creator?.name || '-'}</td>
                  <td className="px-4 py-2 text-gray-500">{fmt(e.deadline)}</td>
                  <td className="px-4 py-2">
                    <span
                      className={`text-xs px-1.5 py-0.5 rounded ${e.status === 'active' ? 'bg-blue-100 text-blue-700' : e.status === 'cancelled' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600'}`}
                    >
                      {e.status === 'active'
                        ? '募集中'
                        : e.status === 'cancelled'
                  ...
                  <td className="px-4 py-2">
                    <button onClick={() => del(e)} className="text-xs text-red-600 hover:underline">
                      削除
                    </button>
                  </td>
                </tr>
